...
const Dashboard = () => {
  ...
  // Auto-refresh volunteer application status for volunteers
  useEffect(() => {
    if (!authLoading && currentUser && isVolunteer() && !isAdmin()) {
      const interval = setInterval(() => {
        console.log('Auto-refreshing volunteer application status...');
        refreshVolunteerApplicationStatus();
  ...
  const refreshVolunteerApplicationStatus = async () => {
    try {
      // Check if there were recent local changes (within last 60 seconds)
      const now = Date.now();
      const localUpdateTime = localStorage.getItem('volunteer_app_last_update');
      if (localUpdateTime && (now - parseInt(localUpdateTime)) < 60000) {
        console.log('Skipping auto-refresh due to recent local changes');
        return;
      }

      const backendVolunteerStatus = await apiService.getVolunteerStatus();
      console.log('Refreshed volunteer status:', backendVolunteerStatus);

      if (backendVolunteerStatus && backendVolunteerStatus.volunteerEmployeeStatus === 'NOT_REGISTERED') {
        const notRegisteredStatus = {
          status: 'not_registered',
          message: 'Apply for team positions to access additional volunteer features'
        };
        setVolunteerApplicationStatus(notRegisteredStatus);
        setCurrentVolunteerApplication(null);
      } else if (backendVolunteerStatus && backendVolunteerStatus.volunteerEmployeeStatus !== 'NOT_REGISTERED') {
        ...
        const hasApproved = teamApplications.some(ta => ta.status?.toUpperCase() === 'APPROVED');

        let applicationStatus = 'pending';
        if (allHaveFeedback) {
          applicationStatus = hasApproved ? 'approved' : 'denied';
        }

        const volunteerApplicationFromBackend = {
          selectedCategories: teamApplications.map(ta => ta.teamName.toLowerCase().replace(/\s+/g, '-')),
        ...
        // Transform backend data to frontend structure for consistency
        ...
        setCurrentVolunteerApplication(frontendApplication);
      }
    } catch (error) {
      console.error('Error refreshing volunteer application status:', error);
    }
  };
    ...
    try {
      ...
      if (userIsAdmin) {
        ...
        try {
          ...
          console.log('Volunteer employee status:', backendVolunteerStatus?.volunteerEmployeeStatus);
          console.log('Team applications:', backendVolunteerStatus?.teamApplications);

          if (backendVolunteerStatus && backendVolunteerStatus.volunteerEmployeeStatus === 'NOT_REGISTERED') {
            // Handle not registered state - show team application prompt
            const notRegisteredStatus = {
              status: 'not_registered',
              message: 'Apply for team positions to access additional volunteer features'
            };
            console.log('Setting NOT_REGISTERED status:', notRegisteredStatus);
            setVolunteerApplicationStatus(notRegisteredStatus);
            setCurrentVolunteerApplication(null);
          } else if (backendVolunteerStatus && backendVolunteerStatus.volunteerEmployeeStatus !== 'NOT_REGISTERED') {
            // Build status from backend data
            ...
            const frontendApplication = {
              selectedCategories: backendVolunteerStatus.teamApplications?.map(ta => ta.teamName.toLowerCase().replace(/\s+/g, '-')) || [],
              submittedAt: backendVolunteerStatus.registrationDate,
              status: applicationStatus,
              decisionsByRole: decisionsByRole,
              teamApplications: backendVolunteerStatus.teamApplications // Keep original for reference
            };

            console.log('DEBUG: Auto-refresh setting volunteer status from backend:', volunteerStatus);
            console.log('DEBUG: Auto-refresh setting current application from backend:', frontendApplication);
            setVolunteerApplicationStatus(volunteerStatus);
            setCurrentVolunteerApplication(frontendApplication);

            // Clear any stale localStorage data to prevent conflicts
            if (currentUser?.uid) {
              localStorage.removeItem(`volunteer_draft_${currentUser.uid}`);
            }
          } else {
            console.log('No backend volunteer status found during auto-refresh - NOT falling back to localStorage');
            // During auto-refresh, don't fall back to localStorage as it may be stale
            // Only clear state if we're certain there's no backend data
            setCurrentVolunteerApplication(null);
                  ...
                  <div className="tab-content ">
                    ...
                    {volunteerEvents.length === 0 ? (
                      <div className="empty-state">
                        ...
                        <p>You are not currently signed up for any volunteer events.</p>
